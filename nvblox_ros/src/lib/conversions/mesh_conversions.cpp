...
void meshBlockMessageFromMeshBlock(
  const CertifiedMeshBlock & mesh_block, nvblox_msgs::msg::MeshBlock * mesh_block_msg)
{
  CHECK_NOTNULL(mesh_block_msg);
  ...
  const std::vector<Index3D> & block_indices_to_delete)
{
  ...
  for (size_t i = 0; i < block_indices.size(); i++) {
    ...
    CertifiedMeshBlock::ConstPtr mesh_block =
      mesh_layer.getBlockAtIndex(block_indices[i]);
    if (mesh_block == nullptr) {
      continue;
    }
...
void markerMessageFromMeshBlock(const CertifiedMeshBlock::ConstPtr& mesh_block,
                                const std::string& frame_id,
                                visualization_msgs::msg::Marker* marker,
                                bool minimal_msg) {
  marker->header.frame_id = frame_id;
...
void markerMessageFromMeshLayer(
  ...
  size_t output_index = 0;
  for (size_t i = 0; i < indices.size(); i++) {
    CertifiedMeshBlock::ConstPtr mesh_block = mesh_layer.getBlockAtIndex(indices[i]);
    if (mesh_block->size() == 0) {
      continue;
    }
    markerMessageFromMeshBlock(mesh_block, frame_id,
