...
void CertifiedEsdfSliceConverter::populateSliceFromLayer(
...
void CertifiedEsdfSliceConverter::distanceMapSliceImageFromLayer(
    const CertifiedEsdfLayer& layer, float z_slice_level,
    const AxisAlignedBoundingBox& aabb, Image<float>* map_slice_image_ptr) {
  CHECK_NOTNULL(map_slice_image_ptr);

  const float voxel_size =
      layer.block_size() / VoxelBlock<CertifiedEsdfVoxel>::kVoxelsPerSide;
...
void CertifiedEsdfSliceConverter::distanceMapSliceImageToMsg(
...
CertifiedEsdfSliceConverter::getBoundingBoxOfLayerAtHeight(
    const CertifiedEsdfLayer& layer, const float z_slice_level) {
...
void CertifiedEsdfSliceConverter::distanceMapSliceFromLayer(
    const CertifiedEsdfLayer& layer, float z_slice_level,
    nvblox_msgs::msg::DistanceMapSlice* map_slice) {
  CHECK_NOTNULL(map_slice);
...
void CertifiedEsdfSliceConverter::distanceMapSliceFromLayers(
    const CertifiedEsdfLayer& layer_1, const CertifiedEsdfLayer& layer_2,
...
// pixel if required.
__global__ void sliceImageToPointcloudKernelCert(
...
void CertifiedEsdfSliceConverter::sliceImageToPointcloud(
    const Image<float>& map_slice_image, const AxisAlignedBoundingBox& aabb,
    float z_slice_level, float voxel_size,
  ...
  const dim3 num_blocks(
      ...
      1);
  sliceImageToPointcloudKernelCert<<<num_blocks, kThreadsPerThreadBlock, 0,
                                     cuda_stream_>>>(
